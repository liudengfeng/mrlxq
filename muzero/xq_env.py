...
class XQEnvBase(gym.Env):
    ...
    def __init__(self, config: EnvContext):
        ...
        # 是否生成棋谱
        self.gen_qp = config.get("gen_qp", False)
        # 初始化中文棋谱
        if self.gen_qp:
            self._init_cn_qipu()
    ...
    def _fix_action_mask(self, obs):
        ...
        self.valid_actions = np.zeros(self.action_space.n, dtype=np.float32)
        for a in legal_actions:
            self.valid_actions[a] = 1.0
...
class XiangQiV1(XQEnvBase):
    ...
    def _make_observation_space(self):
        self.observation_space = Dict(
            {
                # [a,b]
                # Gymnasium recommend flattening the obs to have only a 1D vector
                "s": Box(
                    -NUM_PIECE,
                    NUM_PIECE,
                    (NUM_ROW * NUM_COL,),
                ...
                "steps": Discrete(MAX_EPISODE_STEPS + 1),
                "continuous_uneaten": Discrete(MAX_NUM_NO_EAT + 1),
                "to_play": Discrete(NUM_PLAYER, start=1),
            }
        )
